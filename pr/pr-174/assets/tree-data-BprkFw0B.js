...
`,Ve=["type","pattern","allowed-pattern","min","max","step","autosize","label",...Wt],Be=s=>{const{type:t="text",pattern:e,allowedPattern:i,autocomplete:r,value:n,readonly:o,disabled:l,min:c,max:a,step:v,maxlength:u}=s,{onChange:p,onFocus:f,onInput:g,onRef:_}=jt(s),d=Me(i);return Ht({focus:()=>s.shadowRoot?.querySelector("#input")?.focus()},[]),Ut(E`
			<input
				${mt(_)}
		...
		`,s)};customElements.define("cosmoz-input",_t(Be,{observedAttributes:Ve,styleSheets:[ft(Gt)]}));const kt=s=>{s.style.height="",s.style.height=`${s.scrollHeight}px`},Fe=(s,t=0)=>{if(t>0){const e=s.getAttribute("rows")??"",i=s.style.height;s.style.height="",s.setAttribute("rows",t),s.style.maxHeight=s.getBoundingClientRect().height+"px",s.style.height=i,s.setAttribute("rows",e)}},He=s=>{const{value:t,maxRows:e}=s,i=R(()=>()=>s.shadowRoot.querySelector("#input"),[]);S(()=>Fe(i(),e),[e,i]),S(()=>kt(i()),[i,t]),S(()=>{const r=i(),n=new ResizeObserver(()=>requestAnimationFrame(()=>kt(r)));return n.observe(r),()=>n.unobserve(r)},[i])},je=["rows",...Wt],Ue=s=>{const{autocomplete:t,value:e,placeholder:i,readonly:r,disabled:n,rows:o,cols:l,maxlength:c}=s,{onChange:a,onFocus:v,onInput:u,onRef:p}=jt(s);return He(s),Ut(E`
			<textarea id="input" part="input"
				...
				.value=${Ft(e??"")} maxlength=${L(c)} @input=${u}
...
`,Ke=({tree:s,searchPlaceholder:t,searchGlobalPlaceholder:e,searchMinLength:i=3,opened:r,searchDebounceTimeout:n=2e3})=>{const o=Dt(),l=Re(),[c,a]=q("highlightedNode"),[v,u]=q("nodesOnNodePath",[]),[p,f]=ht(""),[g,_]=ht(""),[d,m]=ht(""),[y,C]=q("selectedNode"),[it,gt]=q("nodePath","");S(()=>{const h=setTimeout(()=>{const b=g.trim().length;b>0&&b<i||f(g.trim())},n);return()=>clearTimeout(h)},[g]);const rt=R(()=>qe(s,p,d),[s,p,d]),vt=x(h=>{if(!h?.pathLocator||!s){u([]);return}const b=At(h.pathLocator,s);u(b)},[s]),U=x(h=>{m(h?.pathLocator||""),_(""),a(null),gt(""),u([])},[]),nt=x(h=>{h&&(a(h),C(h),gt(h.pathLocator??""),vt(h))},[vt]);S(()=>{if(!v?.length||!s||!r)return;const h=v[v.length-1];if(!h?.pathLocator)return;if(s.hasChildren(h)){m(h.pathLocator);return}const b=ut(s,h);m(b),a(h)},[v,s,r]),S(()=>{pt(l,"highlightedNodePath",c?.pathLocator||"")},[c]),S(()=>{y&&y!==c&&a(y)},[y]),S(()=>{if(!(it||s))return;const h=At(it,s);u(h);const b=h.length>0?h[h.length-1]:null;b&&b!==c&&a(b)},[it,s]);const I=We({dataPlane:rt,highlightedNode:c??null,onNodeClick:U});S(()=>{if(!r)return;const h=()=>{const w=o.current;return w?w[Object.getOwnPropertySymbols(w)[0]]:null},b=h();if(b&&I.highlightedNode){const w=I.dataPlane?.indexOf(I.highlightedNode);w!==void 0&&w>=0&&(b.scrollToIndex={index:w,position:"center"})}const D=w=>{if(w.ctrlKey&&w.altKey||w.defaultPrevented)return;const{dataPlane:W,highlightedNode:O,onNodeClick:Qt}=I,G=h();if(!G)return;const ot=W.findIndex(P=>P.pathLocator===O?.pathLocator),bt=(P,yt)=>P>=0&&P<W.length?(a(W[P]),(yt==="start"?P<G._firstVisible:P>G._lastVisible)&&(G.scrollToIndex={index:P,position:yt}),!0):!1;switch(w.key){case"Up":case"ArrowUp":{w.preventDefault(),bt(Math.max(ot-1,0),"start");break}case"Down":case"ArrowDown":{w.preventDefault(),ot<W.length-1&&bt(ot+1,"end");break}case"Enter":w.preventDefault(),O&&(l.dispatchEvent(new CustomEvent("node-dblclicked")),nt(O));break;case"Right":case"ArrowRight":w.preventDefault(),O&&s.hasChildren(O)&&Qt(O);break}};return document.addEventListener("keydown",D,!0),()=>document.removeEventListener("keydown",D,!0)},[r,I,nt]);const qt=h=>b=>{Qe(b,l),nt(h)},Yt=(h,b)=>h?E` <div class="item">
			${(D=>M(p&&(b===0||D!==ut(s,rt[b-1])),()=>E`
						<div class="section">
