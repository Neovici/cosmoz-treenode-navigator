 ...
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */const zt=(s,t,e)=>{const i=new Map;for(let r=t;r<=e;r++)i.set(s[r],r);return i},Ce=j(class extends et{constructor(s){if(super(s),s.type!==z.CHILD)throw Error("repeat() can only be used in text expressions")}dt(s,t,e){let i;e===void 0?e=t:t!==void 0&&(i=t);const r=[],n=[];let o=0;for(const l of s)r[o]=i?i(l,o):o,n[o]=e(l,o),o++;return{values:n,keys:r}}render(s,t,e){return this.dt(s,t,e).values}update(s,[t,e,i]){const r=Kt(s),{values:n,keys:o}=this.dt(t,e,i);if(!Array.isArray(r))return this.ut=o,n;const l=this.ut??=[],a=[];let h,v,u=0,p=r.length-1,f=0,g=n.length-1;for(;u<=p&&f<=g;)if(r[u]===null)u++;else if(r[p]===null)p--;else if(l[u]===o[f])a[f]=T(r[u],n[f]),u++,f++;else if(l[p]===o[g])a[g]=T(r[p],n[g]),p--,g--;else if(l[u]===o[g])a[g]=T(r[u],n[g]),V(s,a[g+1],r[u]),u++,g--;else if(l[p]===o[f])a[f]=T(r[p],n[f]),V(s,r[u],r[p]),p--,f++;else if(h===void 0&&(h=zt(o,f,g),v=zt(l,u,p)),h.has(l[u]))if(h.has(l[p])){const _=v.get(o[f]),d=_!==void 0?r[_]:null;if(d===null){const m=V(s,r[u]);T(m,n[f]),a[f]=m}else a[f]=T(d,n[f]),V(s,r[u],d),r[_]=null;f++}else ht(r[p]),p--;else ht(r[u]),u++;for(;f<=g;){const _=V(s,a[g+1]);T(_,n[f]),a[f++]=_}for(;u<=p;){const _=r[u++];_!==null&&ht(_)}return this.ut=o,Nt(s,a),$}});/**
 * @license
 * Copyright 2021 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */const Ft=j(class extends et{constructor(s){if(super(s),s.type!==z.PROPERTY&&s.type!==z.ATTRIBUTE&&s.type!==z.BOOLEAN_ATTRIBUTE)throw Error("The `live` directive is not allowed on child or event bindings");if(!Tt(s))throw Error("`live` bindings can only contain a single expression")}render(s){return s}update(s,[t]){if(t===$||t===H)return t;const e=s.element,i=s.name;if(s.type===z.PROPERTY){if(t===e[i])return $}else if(s.type===z.BOOLEAN_ATTRIBUTE){if(!!t===e.hasAttribute(i))return $}else if(s.type===z.ATTRIBUTE&&e.getAttribute(i)===t+"")return $;return Nt(s),t}});/**
 * @license
 * Copyright 2018 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
...
`,Ve=["type","pattern","allowed-pattern","min","max","step","autosize","label",...Wt],Be=s=>{const{type:t="text",pattern:e,allowedPattern:i,autocomplete:r,value:n,readonly:o,disabled:l,min:a,max:h,step:v,maxlength:u}=s,{onChange:p,onFocus:f,onInput:g,onRef:_}=jt(s),d=Me(i);return Ht({focus:()=>s.shadowRoot?.querySelector("#input")?.focus()},[]),Ut(E`
		...
		`,s)};customElements.define("cosmoz-input",mt(Be,{observedAttributes:Ve,styleSheets:[_t(Gt)]}));const kt=s=>{s.style.height="",s.style.height=`${s.scrollHeight}px`},Fe=(s,t=0)=>{if(t>0){const e=s.getAttribute("rows")??"",i=s.style.height;s.style.height="",s.setAttribute("rows",t),s.style.maxHeight=s.getBoundingClientRect().height+"px",s.style.height=i,s.setAttribute("rows",e)}},He=s=>{const{value:t,maxRows:e}=s,i=R(()=>()=>s.shadowRoot.querySelector("#input"),[]);S(()=>Fe(i(),e),[e,i]),S(()=>kt(i()),[i,t]),S(()=>{const r=i(),n=new ResizeObserver(()=>requestAnimationFrame(()=>kt(r)));return n.observe(r),()=>n.unobserve(r)},[i])},je=["rows",...Wt],Ue=s=>{const{autocomplete:t,value:e,placeholder:i,readonly:r,disabled:n,rows:o,cols:l,maxlength:a}=s,{onChange:h,onFocus:v,onInput:u,onRef:p}=jt(s);return He(s),Ut(E`
			<textarea id="input" part="input"
				...
				.value=${Ft(e??"")} maxlength=${L(a)} @input=${u}
