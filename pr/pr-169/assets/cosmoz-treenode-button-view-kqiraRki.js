import{c as H,u as h,b as V,d as E,e as x,f as T,g as _,h as J,i as O,n as Q,l as C}from"./tree-data-DwZ1bOnv.js";import{x as m}from"./iframe-CZzJgubh.js";const U=H`
	h1,
	h2 {
		font-weight: 500;
	}
...
`,W=(t,i)=>{const s=a=>{if(a.key===t&&i instanceof Function)return a.preventDefault(),i()};h(()=>(document.addEventListener("keydown",s),()=>{document.removeEventListener("keydown",s)}))},X=(t,i)=>{let s;const a=[];return(...w)=>new Promise((v,y)=>{clearTimeout(s),s=setTimeout(()=>{const u=[...a];a.length=0,Promise.resolve(t(...w)).then(c=>{u.forEach(({resolve:n})=>n(c))},c=>{u.forEach(({reject:n})=>n(c))})},i),a.push({resolve:v,reject:y})})},D=({tree:t,multiSelection:i=!1,dialogText:s,buttonTextPlaceholder:a,searchPlaceholder:w,noReset:v=!1,searchGlobalPlaceholder:y,searchMinLength:u=3,searchDebounceTimeout:c=2e3})=>{const n=E(null),k=E(null),[o,g]=x("highlightedNode"),[d,$]=x("selectedNode"),[f,N]=x("nodePath",""),[l,b]=x("nodesOnNodePath",[]),[R,L]=T(!1),[p,z]=T([]);h(()=>{n.current&&(k.current=n.current.querySelector("cosmoz-treenode-navigator")?.shadowRoot?.querySelector("cosmoz-input"))},[n.current]),h(()=>{o!==d&&$(o),o?.pathLocator!==f&&N(o?.pathLocator??"")},[o]),h(()=>{d&&d!==o&&g(d)},[d]),h(()=>{if(!(f||t))return;const e=_(f,t);b(e);const r=e.length>0?e[e.length-1]:null;r&&r!==o&&g(r)},[f,t]);const A=J(()=>!Array.isArray(l)||l.length===0?a||"":l.filter(e=>e).map(e=>e[t.searchProperty]).join(" / "),[l,t]),I=()=>{b([]),z([]),g(null),$(null),N("")},M=({item:e,ev:r})=>{z(p.filter(G=>G!==e)),r.preventDefault(),r.stopPropagation()},j=e=>e.name,q=(e,r)=>e&&r>0,B=X(()=>{n.current?.fit?.()},50),K=()=>{k.current&&k.current.focus()},F=()=>{n.current?.showModal(),L(!0),setTimeout(K,0)},P=()=>{L(!1),n.current?.close()};W("Escape",P);const S=()=>{o?.pathLocator&&(i&&!p.some(e=>e.pathLocator===o.pathLocator)&&z([...p,o]),$(o),N(o.pathLocator),b(_(o.pathLocator,t)),P())};return m`
