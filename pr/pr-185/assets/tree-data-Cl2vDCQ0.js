...
`;customElements.define("cosmoz-toggle",tt(Ge,{styleSheets:[qe,Ye],observedAttributes:["disabled"]}));const Qe=s=>{const t=O(()=>({}),[]);return O(()=>Object.assign(t,s),[t,...Object.values(s)])},dt=(s,t)=>{const e=t.pathLocator||t.path||"",{pathLocatorSeparator:i}=s;return e.includes(i)?e.substring(0,e.lastIndexOf(i)):e},Je=(s,t)=>{if(!s)return;const e=s.getNodeByPathLocator(t),i=e?s.getChildren(e):[],{searchProperty:r}=s,n=(o,l)=>{if(s.hasChildren(o)){if(!s.hasChildren(l))return-1}else if(s.hasChildren(l))return 1;const c=o[r],a=l[r];return c>a?1:c<a?-1:0};return i.length>0?i.sort(n):e},Ke=(s,t,e)=>{if(!s)return[];const i=Je(s,e);return t?s.searchNodes(t,i,!1):i},Ze=(s,t,e)=>{let i=!1;return e&&(i=t===e||t.id&&t.id===e.id||t.pathLocator===e.pathLocator),i?`${s} selected`:s},Xe=(s,t)=>{t.dispatchEvent(new CustomEvent("node-dblclicked",{detail:{model:s.model}}))},Nt=(s,t)=>!t||!s?[]:t.getPathNodes(s).filter(e=>e!==void 0),ts=mt`
	:host {
...
`,es=({tree:s,searchPlaceholder:t,searchGlobalPlaceholder:e,searchMinLength:i=3,opened:r,searchDebounceTimeout:n=2e3})=>{const o=Bt(),l=Me(),[c,a]=Y("highlightedNode"),[v,u]=Y("nodesOnNodePath",[]),[p,f]=ct(""),[g,_]=ct(""),[d,m]=ct(""),[y,x]=Y("selectedNode"),[rt,qt]=Y("nodePath","");E(()=>{const h=setTimeout(()=>{const b=g.trim().length;b>0&&b<i||f(g.trim())},n);return()=>clearTimeout(h)},[g]);const nt=O(()=>Ke(s,p,d),[s,p,d]),bt=S(h=>{if(!h?.pathLocator||!s){u([]);return}const b=Nt(h.pathLocator,s);u(b)},[s]),U=S(h=>{m(h?.pathLocator||""),_(""),a(null)},[]),ot=S(h=>{h&&(a(h),x(h),qt(h.pathLocator??""),bt(h))},[bt]);E(()=>{if(!v?.length||!s||!r)return;const h=v[v.length-1];if(!h?.pathLocator)return;if(s.hasChildren(h)){m(h.pathLocator);return}const b=dt(s,h);m(b),a(h)},[v,s,r]),E(()=>{ft(l,"highlightedNodePath",c?.pathLocator||"")},[c]),E(()=>{y&&y!==c&&a(y)},[y]),E(()=>{if(!(rt||s))return;const h=Nt(rt,s);u(h);const b=h.length>0?h[h.length-1]:null;b&&b!==c&&a(b)},[rt,s]);const I=Qe({dataPlane:nt,highlightedNode:c??null,onNodeClick:U});E(()=>{if(!r)return;const h=()=>{const w=o.current;return w?w[Object.getOwnPropertySymbols(w)[0]]:null},b=h();if(b&&I.highlightedNode){const w=I.dataPlane?.indexOf(I.highlightedNode);w!==void 0&&w>=0&&(b.scrollToIndex={index:w,position:"center"})}const D=w=>{if(w.ctrlKey&&w.altKey||w.defaultPrevented)return;const{dataPlane:W,highlightedNode:M,onNodeClick:Kt}=I,G=h();if(!G)return;const lt=W.findIndex(A=>A.pathLocator===M?.pathLocator),yt=(A,wt)=>A>=0&&A<W.length?(a(W[A]),(wt==="start"?A<G._firstVisible:A>G._lastVisible)&&(G.scrollToIndex={index:A,position:wt}),!0):!1;switch(w.key){case"Up":case"ArrowUp":{w.preventDefault(),yt(Math.max(lt-1,0),"start");break}case"Down":case"ArrowDown":{w.preventDefault(),lt<W.length-1&&yt(lt+1,"end");break}case"Enter":w.preventDefault(),M&&(l.dispatchEvent(new CustomEvent("node-dblclicked")),ot(M));break;case"Right":case"ArrowRight":w.preventDefault(),M&&s.hasChildren(M)&&Kt(M);break}};return document.addEventListener("keydown",D,!0),()=>document.removeEventListener("keydown",D,!0)},[r,I,ot]);const Qt=h=>b=>{Xe(b,l),ot(h)},Jt=(h,b)=>h?C` <div class="item">
		...
		<div class="header">
			<h3 class="path">
				...
				${Ce([s,d],()=>(d&&s?.getPathNodes(d)||[]).filter(h=>h!==void 0).map(h=>C`
		...
		${$(s,()=>C` <div
				...
				>
					...
					${Re({items:nt,renderItem:Jt,scroller:!0})}
				</div>`)}
		${$(p&&d,()=>C`
