...
import { TreeNode, TreenodeNavigatorProps } from './util/types';
import styles                               from './cosmoz-treenode-navigator.styles.js';
...
const TreenodeNavigator = ({
	...
	const listRef = useRef<HTMLDivElement & { [key: symbol]: any }>();
	const host = useHost();
	...
	const meta = useMeta<Meta>({ dataPlane, highlightedNode, onNodeClick });
	...
	useEffect(() => {
		...
		const handler = (e: KeyboardEvent) => {
			...
			const navigateToIndex = (newIndex: number, position: 'start' | 'end') => {
	...
	const renderItem = (node: TreeNode | undefined, index: number) => {
		if (!node) {
			return html`<div></div>`;
		}
	...
	return html`
		...
		${when(
			tree,
			() =>
				html` <div class="items" ${ref((el) => (listRef.current = el as any))}>
